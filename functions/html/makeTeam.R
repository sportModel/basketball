makeTeam <- function(raw,tf,vc,type="html",teams=par@team,filename)
  {
    if (type=="pdf")
      {
        sink(filename)
    ...
    for (team in teams)
      {
        ...
        if (par@level=="ncaa") display.categories.raw <- c("Yr","No","Pos","G","MP","FG","FGA","3P","3PA","FT","FTA","ORB","DRB","AST","TOV","STL","BLK","PTS")
        else display.categories.raw <- c("Pos","G","MP","FG","FGA","3P","3PA","FT","FTA","ORB","DRB","AST","TOV","STL","BLK","PTS")
        ...
        if (par@level=="ncaa") display.digits.raw <- c(0,0,0,0,rep(1,length(display.categories.raw)-3))
        else display.digits.raw <- c(0,0,0,rep(1,length(display.categories.raw)-2))
        
        display.digits.tf <- c(0,0,rep(1,7),2,2,1,1,2,2)
        display.digits.vc1 <- c(0,rep(2,length(display.categories.vc1)))
        display.digits.vc2 <- c(0,rep(2,length(display.categories.vc2)))
        ...
        if (type=="html")
          {
            ...
            sink(filename)
            cat(raw$Team.Opp$FullName[which(team==raw$Team.Opp$Team & raw$Team.Opp$Team.Opp=="Team")],"<br>\n")
