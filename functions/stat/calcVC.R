calcVC <- function(tf,raw,game=FALSE) {
  ...
  for (i in 1:5) {
    ...
    VC.2 <- tf[,"M2100"] * (2 - model$eP) * (1 - 0.5 * tf[,"AssistRate"]) + tf[,"Miss2100"] * model$eP * (model$eR - 1)
    VC.3 <- tf[,"M3100"] * ((3 - model$eP) - (0.5 * tf[,"AssistRate"]) * (2 - model$eP)) + tf[,"Miss3100"] * model$eP * (model$eR - 1)
