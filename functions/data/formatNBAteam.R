formatNBAteam <- function(team,standings)
  {
    ...
    full.team.name <- raw[grep("<title>",raw,fixed=TRUE)]
    full.team.name <- gsub(".*-[0123456789][0123456789] ","",full.team.name)
    full.team.name <- gsub(" Roster and.*","",full.team.name)
    raw <- extractTable(raw,"roster")
    raw <- scrub(raw)
    roster <- matrix(raw,ncol=8,byrow=T)
    ...
    ## totals
    raw <- readLines(paste("data/nba/",par@year,"/raw/",team,".html",sep=""),warn=F)
    raw <- extractTable(raw,"totals")
    raw <- scrub(raw)
    totals <- matrix(raw,ncol=24,byrow=T)
