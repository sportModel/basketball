...
formatGame <- function(filename)
  {
    ...
    if (par@level=="ncaa")
      {
        raw <- cbind(Name,Team=c(rep(1,n1),rep(2,n2)),Pos,pos,"","","","",1,NA,raw[,2],splitter(raw[,3]),splitter(raw[,4]),splitter(raw[,5]),raw[,6],-1,raw[,7],raw[,8],raw[,11],raw[,9],raw[,10],raw[,13])
        colnames(raw) <- c("Name","Team","Pos","pg","sg","sf","pf","c","No","Yr","Ht","Wt","G","GS","MP","FG","FGA","3P","3PA","FT","FTA","ORB","DRB","tot","AST","TOV","STL","BLK","PTS")
        raw <- as.data.frame(raw)
        raw$DRB <- as.numeric(raw$tot)-as.numeric(raw$ORB)
        raw <- raw[,-which(names(raw)=="tot")]
      }
    if (par@level=="nba")
      {
